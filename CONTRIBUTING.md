...
`bun run e2e` runs the Playwright suite in `e2e/` against the local stack: start `bun run infra:up` first (Postgres, MailHog, Garage); the dev server is started for you when nothing listens on 5173. Every test context sends its own `x-forwarded-for`, so sign-up's per-address email cap never trips; a dev server you start yourself needs `TRUSTED_PROXY_HEADER=x-forwarded-for` in `.env` (the example sets it). Each run registers its own account through the real sign-up flow, reading the verification link from MailHog, then uploads, previews, downloads, moves, copies, restores an earlier version and locks the way a person would; a second file registers two people who pin each other's fingerprints and checks the operator area, a third shares a folder between two people, uploads into it as an editor, and revokes, and a fourth opens links as a visitor with no account, with and without a password, keeps a copy while signed in, and sees a stopped link stop. Do not edit source files while it runs: the dev server's full reloads reset the page mid-test. Traces of failures land in `test-results/`; open one with `bunx playwright show-trace <trace.zip>`. `bunx playwright install webkit` once, then `bun run e2e -- --project=webkit`, runs the same suite on WebKit, the engine behind Safari. CI runs Chromium and the phone project only: WebKit on the GitHub runner is too slow for the registration step and never fires its download events, so run it locally before touching the worker, key storage or download paths. Use Playwright for any new browser check rather than a one-off script.
...
HushOS 1.0 is Drive. Accounts (email verification, OPAQUE authentication, remembered unlock, recovery, password changes, master/recovery-key rotation, identity keys, quota grants, permanent deletion), Drive (encrypted tree, resumable uploads, streaming downloads, previews, thumbnails, versions, trash, worker sweeps), sharing (contacts, shares to accounts, links, save a copy, rotation on revoke, change feeds), reports with an evidence pipeline, Polar billing, and the security headers are implemented and covered by the unit and browser suites. The rest of the productivity suite, chat first, builds on the same packages. HushOS is licensed under the [GNU Affero General Public License v3.0 only](LICENSE) (`AGPL-3.0-only`).
...
`@hushos/auth/server` owns account enrollment, sessions, and authorization checks; its functions take session or enrollment tokens and plain values, never a request. `@hushos/auth/http` is the only place auth touches HTTP: it reads and writes the token cookies, resolves the client address, and guards browser-originated mutations, so API handlers and server loaders go through it. `@hushos/auth/client` orchestrates the browser flow through `CryptoTransport`, `DeviceKeyStore` and `AuthApi` adapters; web routes own the UI. `AuthApi` (`packages/auth/src/api.ts`) is the contract the client needs from the server; the web app implements it in `apps/web/src/lib/auth-api.ts` with an Eden Treaty client typed by the Elysia app, so a route or response change that the client does not expect fails `bun run typecheck` there. Error handlers in `api.server.ts` must return literal status codes: a plain `number` erases every inferred response type. `@hushos/drive` follows the same split: `./server` is the tree and upload domain the Elysia handlers call, `./client` opens nodes through the crypto worker and turns intent into envelopes, `./transfers` is the upload queue that runs across pages, `./downloads` is the download engine (ordered ranged reads decrypted in the worker, written to a sink the app supplies, zipped for folders, plus a cached random-access reader the viewers use), `./journal` is the IndexedDB upload journal, `./storage` is the S3 client, and `./api` and `./protocol` are the contract and constants a native client would share. Drive's crypto primitives live in `@hushos/crypto/drive`. `@hushos/crypto` owns the protocol versions, OPAQUE client operations, HKDF, and the account-key and workspace-grant envelopes. Its crypto session has no React, HTTP, or worker dependency. Other clients can reuse the protocol and primitives where WebAssembly/Web Crypto are available; native mobile runtime integration is still required. Server OPAQUE primitives have a separate `@hushos/crypto/server` export.
