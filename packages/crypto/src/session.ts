...
export type CryptoRequests = {
    ...
    driveSealReport: {
        ...
        keyEpoch: number;
        reportId: string;
        operators: { userId: string; publicKey: string }[];
    };
    driveOpenReport: {
        workspaceId: string;
        nodeId: string;
        ...
        switch (message.operation) {
            ...
            case 'driveSealReport': {
                ...
                for (const operator of message.input.operators)
                    envelopes.push({
                        ...
                        keyEnvelope: encode(
                            await sealReportKey(node.key, decode(operator.publicKey, 32), {
                                workspaceId,
                                nodeId,
                                keyEpoch,
                                reportId,
            ...
            case 'driveOpenReport': {
                ...
                const key = await openReportKey(
                    decode(message.input.keyEnvelope, 112),
                    decode(identityKey.publicKey, 32),
                    identityKey.key,
