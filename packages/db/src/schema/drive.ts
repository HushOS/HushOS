...
/* The node key sealed to one operator: a sealed box (112 bytes) opened only under that operator's identity. */
export const driveReportKeys = pgTable(
    ...
    (table) => [
        ...
        check('drive_report_keys_envelope_valid', sql`octet_length(${table.keyEnvelope}) = 112`),
    ],
);
