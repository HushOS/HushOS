...
describe('the sweeps', () => {
    ...
    test('tombstones go leaves first once their window has passed, live rows never', async () => {
        ...
        // What a client's cursor has to know: the feed at a cursor older than the
        // dropped tombstone says start over; at or past it, and from zero, it does not.
        const dropped = (
            await db
                .select({ seq: driveNodes.changeSeq })
                .from(driveNodes)
                .where(eq(driveNodes.id, inner.id))
        )[0]!.seq!;
        expect(await drive.deleteExpiredTombstones(90)).toBe(1);
        expect(
            (await drive.listChanges({ workspaceId: account.workspaceId, since: dropped - 1 }))
                .resync,
        ).toBe(true);
        expect(
            (await drive.listChanges({ workspaceId: account.workspaceId, since: dropped })).resync,
        ).toBe(false);
        expect(
            (await drive.listChanges({ workspaceId: account.workspaceId, since: 0 })).resync,
        ).toBe(false);
        expect(await drive.deleteExpiredTombstones(90)).toBe(1);
        expect(await drive.deleteExpiredTombstones(90)).toBe(0);
        const [root] = await db
