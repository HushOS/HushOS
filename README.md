...
Private storage for your files, personal or work, that is simple to use. HushOS is an open-source, self-hostable, end-to-end encrypted productivity suite; version 1 is Drive. Files are encrypted on the person's device, the service holds only ciphertext and the shape of the tree, and the whole thing runs at [hushos.com](https://hushos.com) or on a server you control.

## What it does

- **Accounts.** Email verification, OPAQUE sign-in (the password never leaves the device), remembered device unlock, a 24-word recovery phrase, password changes, master and recovery-key rotation, permanent deletion.
- **Drive.** An encrypted folder tree, resumable uploads straight to an S3-compatible store, streaming downloads and zips decrypted on the device, previews of images, PDF, video, audio, Markdown, code, Word, spreadsheets and CSV, encrypted thumbnails with a grid view, drag and drop, copies, versions, trash, an Info panel that shows an item's ids, sizes and, on request, its keys (downloadable as a key file for decrypting the stored bytes without HushOS), and an installable web app.
- **Sharing.** Contacts pinned by identity key, shares to accounts as viewer or editor, links for anyone with an optional password and expiry, save a copy, a Shared page, key rotation when a share or link is stopped, and change feeds that keep open folders current.
- **Referrals and affiliates.** Every account has an invite link that earns both sides storage, up to a cap; operators enrol creators with a coupon code, a landing page showing the plans with their discount, a Polar discount applied at checkout, and a commission per paid order tracked until it is marked paid.
- **Reports.** Anyone with access can report content; the key is escrowed to the operators' identity keys, the report snapshots the subtree and holds its bytes, an optional evidence bucket keeps a copy, and operators triage, remove, suspend, or file with an authority.
- **Operations.** A pg-boss worker sweeps expired uploads, deletions, replication, purge, usage and object audits, orphans and tiering; Polar bills paid plans on the hosted service and is off by default when self-hosting; every page ships a Content Security Policy.

The detail behind every line above is in the security page, `docs/drive-design.md`, and the code.
...
`/login` unlocks the account key in a worker. A non-exportable Web Crypto device key in IndexedDB protects an encrypted account-key bundle in localStorage through Zustand persist. Refreshes and new tabs restore that key only after checking a live server session and credential revision. Explicit lock or sign-out removes saved device access across tabs. The server session slides: it ends after seven days without a request, and never lasts more than thirty days from sign-in. Browser origin compromise can still misuse a saved device key; non-exportability does not prevent XSS.
...
Browser tests live in `e2e/` and run with `bun run e2e` against the local stack; see CONTRIBUTING.md. conventions

Keep the structure simple. Web pages and layouts live directly in TanStack route files, reusable UI lives in `components/`, and shared helpers live in `lib/`. API handlers live together in `apps/web/src/lib/api.server.ts`, mounted by the TanStack server route `src/routes/api/$.ts`. Extract more modules as needed. Generated `routeTree.gen.ts` is tracked and regenerated by the dev server and the build; CI fails when the committed copy is stale.

`@/*` resolves inside the web app. Cross-workspace imports use public exports such as `@hushos/shared` (consumed by the API handlers) and `@hushos/utils` (a placeholder for shared runtime utilities). They export TypeScript source, which Vite and Bun compile as part of app builds.
