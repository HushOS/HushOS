...
async function signInAgain(p: Page) {
    await p.goto('/app/drive', { waitUntil: 'networkidle' });
...
async function up(p: Page) {
    ...
    await expect(p).toHaveURL(/\/app\/drive\/?$/);
    await expect(row(p, 'Family')).toBeVisible();
}
/* Tags a row by name from the tag dialog: existing names are checked, new ones made. */
async function tag(p: Page, name: string, tags: string[]) {
    const target = row(p, name);
    const box = (await target.boundingBox())!;
    // The middle of the row, in the Modified column: clear of the name and of the uploads flyout at the corner.
    ...
    for (const label of tags) {
        ...
        await p.keyboard.press('Enter');
        await expect(dialog(p).getByRole('checkbox', { name: label })).toBeChecked();
    }
    await dialog(p)
        .getByRole('button', { name: /save tags/i })
        .click();
    await expect(dialog(p)).toHaveCount(0);
    await expect(target).toContainText(tags[0]!);
    await p.mouse.click(box.x + box.width * 0.6, box.y + box.height / 2);
    await expect(target).not.toHaveAttribute('aria-selected', 'true');
    // Past the double-click window, so the next row's click is a click of its own.
    await p.waitForTimeout(600);
}
...
/*
 * The 2x context also yields the 1x picture (one CSS pixel per pixel); the
 * 1.5x picture needs a context of its own, since a screenshot cannot be scaled.
 */
type Scale = 2 | 1.5;
async function shoot(p: Page, name: string, scale: Scale) {
    if (scale === 2) {
...
async function wideShots(p: Page, suffix: string, scale: Scale) {
    ...
    const palette = p.getByPlaceholder(/search files and folders/i);
    await expect(palette).toBeVisible();
    await palette.fill('lis');
    await expect(p.getByRole('option', { name: /^Lisbon 2026/ }).first()).toBeVisible({
        timeout: 30_000,
    });
    await p.waitForTimeout(400);
    await shoot(p, `search${suffix}`, scale);
    ...
    await p.waitForTimeout(400);
    await shoot(p, `preview${suffix}`, scale);
    await p.keyboard.press('Escape');
    await expect(dialog(p)).toHaveCount(0);
    ...
    await p.waitForTimeout(400);
    await shoot(p, `share${suffix}`, scale);
    await p.keyboard.press('Escape');
    await expect(dialog(p)).toHaveCount(0);
    await p.setViewportSize({ width: 1280, height: 800 });
}

async function phoneShots(p: Page, suffix: string, scale: Scale) {
    await signInAgain(p);
    await p.waitForTimeout(600);
    await shoot(p, `drive-narrow${suffix}`, scale);
    await into(p, 'Family');
    await into(p, 'Lisbon 2026');
    const grid = p.getByRole('button', { name: /show as grid/i });
    if (await grid.count()) await grid.click();
...
test('the wide screenshots, light and dark', async ({ browser }) => {
    ...
    for (const [suffix, colorScheme, scale] of [
        ...
        await signInAgain(p);
        await wideShots(p, suffix, scale);
        await context.close();
    }
});
...
test('the phone screenshots, light and dark', async ({ browser }) => {
    ...
    for (const [suffix, colorScheme, scale] of [
        ...
        await phoneShots(await context.newPage(), suffix, scale);
        await context.close();
    }
});
