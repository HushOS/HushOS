...
/*
 * An operator's report page, with its content opened. Every full load restores
 * the device's saved keys before anything renders; when that restore does not
 * come back, the app asks for the password instead, as it would for anyone. A
 * person would type it and carry on, so this does too, and says so. What it
 * will not do is wait out the whole test on a page that is neither: it fails
 * soon, with what the page showed and what the restore logged.
 */
async function openReportContent(page: Page, id: string) {
    const restoreLog: string[] = [];
    const listen = (message: { text(): string }) => {
        if (/Device restore/.test(message.text())) restoreLog.push(message.text());
    };
    ...
    try {
        ...
        try {
            ...
            const shown = (await page.locator('body').innerText())
                ...
                .slice(0, 400);
            throw new Error(
                `The report page never came up. It showed: “${shown}”. Restore log: ${restoreLog.join(' | ') || 'nothing'}`,
            );
        }
...
test.beforeAll(async ({ browser }) => {
    ...
    samples = sampleFiles();
    operator = await (await newContext(browser)).newPage();
