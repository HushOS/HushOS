...
test('a signed-in visitor keeps a copy under their own keys', async ({ browser }) => {
    ...
    await row(keeper, 'payload.bin').locator('button').first().click(grip);
    const [download] = await Promise.all([
...
test('a link made right after stopping one opens, and a stopped link leaves no preview', async ({
    browser,
}) => {
    await owner.goto('/app/drive');
    await expect(row(owner, 'Public')).toBeVisible({ timeout: 60_000 });
