...
export function HotkeyHints() {
    ...
    return (
        <output
            aria-live="polite"
            className="pointer-events-none fixed bottom-4 left-4 z-40 block max-w-sm border bg-popover p-3 text-popover-foreground shadow-hard animate-in fade-in slide-in-from-bottom-1 duration-150 sm:bottom-6 sm:left-6"
        >
...
export function Kbd({ children }: { children: string }) {
    return (
        <kbd className="inline-flex h-5 min-w-5 items-center justify-center border bg-card px-1 font-mono text-[10px] text-foreground">
            {children}
        </kbd>
    );
}
