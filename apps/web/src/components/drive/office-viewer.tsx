...
import { readAll, rememberPreview } from '@/lib/previews';
import {
    clipSheet,
    OFFICE_LIMIT,
    sheetSource,
...
function SheetView({
    ...
    const toggle = source !== null && (
        <div className="flex shrink-0 justify-end border-b border-rule px-3 py-1">
            <Button
                variant="ghost"
                size="xs"
                aria-pressed={raw}
    ...
    if (raw && source !== null)
        return (
            <div className="min-h-0 flex-1 overflow-auto p-3 sm:p-6" data-office="sheet">
                <div className="sheet flex min-w-0 flex-col">
                    {toggle}
    ...
    return (
        <div className="flex min-h-0 flex-1 flex-col p-3 sm:p-6" data-office="sheet">
            <div className="sheet flex min-h-0 flex-col overflow-hidden">
                {toggle}
                {sheets.length > 1 && (
                    <div
                        role="tablist"
                        aria-label="Sheets"
                        className="flex shrink-0 flex-wrap gap-0.5 border-b border-rule bg-muted p-0.5"
                    >
                        {sheets.map((entry, index) => (
                            <button
                                ...
                                className="rounded-xs px-3 py-1.5 text-xs text-muted-foreground outline-none hover:text-foreground focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring aria-selected:bg-card aria-selected:font-semibold aria-selected:text-foreground"
                            >
                                {entry.name}
                            </button>
                        ))}
                        ...
                        <tbody>
                            {cells.map((row, r) => (
                                <tr key={r} className="border-b border-rule">
                                    <th
                                        scope="row"
                                        className="sticky left-0 bg-muted px-2 py-1 text-right font-normal text-muted-foreground"
                                    >
                                        {r + 1}
                                    </th>
                                    {Array.from({ length: width }, (_, c) => (
                                        <td
                                            key={c}
                                            className="max-w-64 truncate border-r border-rule px-2 py-1 whitespace-nowrap"
                                        >
                                            {row[c] ?? ''}
                                        </td>
                                    ))}
                ...
                {(moreRows > 0 || moreCols > 0) && (
                    <p className="shrink-0 border-t border-rule px-4 py-2 text-xs text-muted-foreground">
                        Showing the first {cells.length} rows and {width} columns
                        {moreRows ? `; ${moreRows} more rows` : ''}
                        {moreCols ? `; ${moreCols} more columns` : ''}. Download the file for the
                        rest.
                    </p>
                )}
            </div>
...
function SlidesView({ slides, name }: { slides: Slide[]; name: string }) {
    return (
        <div className="flex-1 overflow-auto px-3 py-4 sm:px-6 sm:py-8" data-office="slides">
            <div className="mx-auto flex max-w-3xl flex-col gap-4">
                ...
                {slides.length === 0 && (
                    <p className="text-sm text-muted-foreground">No slides with text.</p>
                )}
                {slides.map((slide) => (
                    <section
                        key={slide.number}
                        className="sheet px-6 py-5"
