...
export function NavigationBar() {
    ...
    return (
        <div
            aria-hidden="true"
            className={cn(
                'pointer-events-none fixed inset-x-0 top-0 z-50 h-0.5 origin-left bg-primary motion-reduce:transition-none',
                phase === 'idle' && 'scale-x-0 opacity-0',
                phase === 'loading' &&
                    'scale-x-[0.85] opacity-100 transition-transform duration-[8s] ease-out-expo',
                phase === 'done' &&
                    'scale-x-100 opacity-0 transition-[transform,opacity] delay-[0ms,150ms] duration-200 ease-out-soft',
            )}
        />
    );
}
