...
export const driveApi: DriveApi = {
    ...
    document: (workspaceId, kind) =>
        unwrapDrive(api().workspaces({ id: workspaceId }).documents({ kind }).get()),
    putDocument: (workspaceId, kind, input) =>
        unwrapDrive(api().workspaces({ id: workspaceId }).documents({ kind }).put(input)),
