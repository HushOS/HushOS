 ...
 */
export function logoSvg(fill = '#000000', background?: string, radius = 0) {
    const side = 539;
    const left = -60 - (side - 420) / 2;
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${left} -60 ${side} ${side}" width="${side}" height="${side}" fill="none">${background ? `<rect x="${left}" y="-60" width="${side}" height="${side}" rx="${radius}" fill="${background}"/>` : ''}${paths(fill)}</svg>`;
}

/* The square in pixels an SVG from the helpers above rasterises to; wordmarks keep their ratio. */
export const LOGO_SIDE = 539;
export const WORDMARK_SIZE = { width: 1780, height: 539 };
