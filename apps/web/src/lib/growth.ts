...
/* A short identifier under a key; validators sit in the shared bundle, so no schema library here. */
function word<K extends string>(key: K) {
    return (input: unknown): Record<K, string> => {
        const value =
