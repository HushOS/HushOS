...
describe('time left, as read', () => {
    test('is coarse on purpose and switches units at the right places', () => {
        ...
        expect(formatTimeLeft(3569)).toBe('59 min left');
        expect(formatTimeLeft(3600)).toBe('1 h left');
        expect(formatTimeLeft(4200)).toBe('1 h 10 min left');
    });

    test('is held back until the rate has settled, then smoothed toward new readings', () => {
        let s: Smoothed = NO_ESTIMATE;
        ...
        // Losing the rate for a tick, as between two small files, keeps the estimate.
        const held = shownTimeLeft(s);
        for (let i = 0; i < HOLD_TICKS - 1; i++) {
            s = smoothTimeLeft(s, null);
            expect(shownTimeLeft(s)).toBe(held);
        }
        // A reading in between starts the silence over.
        s = smoothTimeLeft(s, 200);
        expect(s.stale).toBe(0);
        // A long silence lets go: the next transfer settles on its own.
        for (let i = 0; i < HOLD_TICKS; i++) s = smoothTimeLeft(s, null);
        expect(shownTimeLeft(s)).toBeNull();
