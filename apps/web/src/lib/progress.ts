 ...
 * 750 ms ticks is not a promise, and "4 min" survives a wobble that "3:47"
 * would not. Short, because it shares a 340px line with the file count.
 */
 ...
 * none. `samples` counts consecutive ticks with a live estimate. A tick
 * without one, which a batch of small files gives between every two files,
 * keeps the last estimate rather than dropping it; only a long silence lets
 * it go, so the line does not blink with the rate.
 */
export type Smoothed = { seconds: number | null; samples: number; stale: number };
export const SETTLE_SAMPLES = 3;
/* Ticks of 750 ms without a reading before the estimate is let go: about fifteen seconds. */
export const HOLD_TICKS = 20;
export const NO_ESTIMATE: Smoothed = { seconds: null, samples: 0, stale: 0 };

export function smoothTimeLeft(previous: Smoothed, next: number | null): Smoothed {
    if (next === null)
        return previous.stale + 1 >= HOLD_TICKS
            ? NO_ESTIMATE
            : { ...previous, stale: previous.stale + 1 };
    const seconds = previous.seconds === null ? next : previous.seconds * 0.7 + next * 0.3;
    return { seconds, samples: previous.samples + 1, stale: 0 };
}
