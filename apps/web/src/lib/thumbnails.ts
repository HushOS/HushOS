...
/* Large enough for a grid tile on a dense screen; the encoder steps down until the result fits the sealed size cap. */
...
async function encode(source: CanvasImageSource, width: number, height: number) {
    ...
    // Size first, then quality: a sharp picture a little smaller beats a large smeared one.
    for (const edge of [1, 0.75, 0.5, 0.375, 0.25].map((step) =>
        Math.round(THUMBNAIL_EDGE * step),
    )) {
