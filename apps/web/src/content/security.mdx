...
The ML-KEM public key is signed by the identity's Ed25519 key, and the server refuses to store one without that signature. It is not folded into the fingerprint, so the fingerprint two people compare stays the X25519 one.
...
The plaintext account key lives only inside a Web Worker. To let a browser reopen your account without the password, the worker wraps the key with a non-exportable AES-256-GCM device key held in IndexedDB. What is persisted is a versioned encrypted bundle and a lock revision; the key itself never enters application state. Locking or signing out clears remembered access across tabs in that browser. The browser also keeps a mirror of your tree in IndexedDB, exactly as the server holds it, ciphertext only, so search and folder listings can be answered on the device; it is opened with your keys each session and cleared when you leave the workspace. Password changes, password recovery, and either key rotation revoke sessions on other browsers too. Those browsers lose access when they next check the session; revocation cannot erase keys already held offline.
...
<Ledger
    rows={[
        ...
        [
            'Metadata',
            'Name, type, size and modified time, encrypted under the node key and padded to 256-byte steps, so its length does not follow the name',
        ],
...
<Ledger
    rows={[
        ...
        [
            'Search',
            'Nothing. Search runs on your device over names it has already decrypted; no query is sent anywhere.',
        ],
        [
            'Tags',
            'That a sealed tag list exists, its size in coarse steps, and when it was last saved; not a name, which items carry a tag, or how many. Tags are never written on an item, so someone you share with sees none of them.',
        ],
